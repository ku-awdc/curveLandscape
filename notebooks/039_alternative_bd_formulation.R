...
#' Alternative formulation can be seen from the paper.
...
tibble(
  ...
  mutate(
    ...
    # birth = mu_baseline + (r_baseline / cc) * pmax(0, cc - N),
    # gamma_param = 1, # in [0, 1]
    #' Proposed scheme in the paper
    # birth = pmax(0, beta_baseline - gamma_param * (r_baseline / cc) * N),
    # death = mu_baseline + (1 - gamma_param) * (r_baseline / cc) * N,
    #' Alternative scheme observed from the paper
    birth = beta_baseline - gamma_param * r_baseline * N / cc,
    death = mu_baseline + (1-gamma_param) * r_baseline * N / cc
    #
  ) %>%
  pivot_longer(c(birth, death),
    names_to = "process_name",
...
# fs::dir_create("figures")
# ggsave(
#   filename = "figures/039_alt_birth_death_rate.svg",
#   device = svglite::svglite,
#   # scaling = 3,
#   scale = 1.3
#   # width = 2*4.27,
...
# fs::dir_create("figures")
# ggsave(
#   filename = "figures/039_alt_birth_death_rate_times_count.svg",
#   device = svglite::svglite,
#   # scaling = 3,
#   scale = 1.3
#   # width = 2*4.27,
...
#' Okay the issue with it is that it can go negative at an arbitrary point.