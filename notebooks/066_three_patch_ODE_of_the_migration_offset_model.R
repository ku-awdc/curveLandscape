...
# pdf("figures/066_three_patch_ODE_migration_offset_wedge.pdf")

carrying_capacity <- c(2, 8, 4)

for (omega_0 in 0:(max(ceiling(carrying_capacity)))) {
  ode_source_only_wedge(
    growth_rate = 4 - 1,
    carrying_capacity = carrying_capacity,
    n0 = c(2, 8 + 4, 0),
    migration_offset = {
      tmp_omega_0 <- if (length(omega_0) == 1) {
        rep.int(omega_0, length(carrying_capacity))
      } else {
        omega_0
      }
      print(
        (carrying_capacity > tmp_omega_0) * (
          tmp_omega_0
        )
      )
    },
    ...
    identity() %>%
    {
      ggplot(.) +
        ...
        labs(
          ...
          y = "Patch-level count",
          x = "time [years]",
          caption = glue("migration offset: {omega_0}"),
        ) +
        geom_hline(
          aes(
