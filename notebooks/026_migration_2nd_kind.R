#'
#' Migration rates m_(ij) is defined as the rate of animals
#' moving from patch j into patch i
#'
#' The total propensity to move from patch j is thus
#'
#' m_(ij) * N_j
#'
#'
#' Migration of the first kind is defined by:
#'
#' m_(ij) = m_0 "kern"("distance"_(ij)") N_j
#'
#' In this script we implement migration in the second kind (source )
#'
#' m_(ij) = m_0 "kern"("distance"_(ij)") / (Slot_j + 1)
#'
#' here Slot_j is defined as (CC_j - N_j)^+
#'
...
tibble(
  N = seq.default(0, to = 18, by = .5)
) %>%
  expand_grid(cc) %>%
  mutate(
    m_pop = N,
    m_cc = N / cc,
    m_slot = N / (1 + pmax(0, cc - N)),
    # m_slot_exp = N * exp(-pmax(0, cc - N))
    # m_slot_exp = N * 2 * exp(-log(1 + exp(cc - N))),
    m_slot_exp = N * exp(-log(1 + exp(cc - N)))
    # m_dest = stop()
  ) %>%
  ...
  identity() %>% {
    ggplot(.) +
      aes(N, value, group = str_c(pattern, cc)) +
      geom_line(
        aes(color = pattern)
      ) +
...
repeat {
  ...
  slots <- pmax(0, cc - current_u)
  # slots <- cc - current_u
  slots_u1 <- slots[c(TRUE, FALSE)]
  slots_u2 <- slots[c(FALSE, TRUE)]
  slots
  migration_rate <- m_baseline *
    kern(distance = dist_01) *
    (current_u1 / (slots_u1 + 1) + current_u2 / (slots_u2 + 1))



  #FIXME: check if any (all?) m_common is zero
  delta_t <- -log(next_unif) / (migration_rate)
  stopifnot(
    length(delta_t) == length(migration_rate)
  )
  ...
  migration_prop <-
    m_baseline * kern(distance = dist_01) *
    cbind(current_u1 / (slots_u1 + 1), current_u2 / (slots_u2 + 1))

  # carrying capacity one
  migration_prop <-
    m_baseline * kern(distance = dist_01) *
    cbind(current_u1 / (cc[1]), current_u2 / (cc[2]))
  migration_prop <-
    m_baseline * kern(distance = dist_01) *
    cbind(current_u1 * exp(-slots_u1), current_u2 * exp(-slots_u2))
  ...
  migration_prop <- rate_to_probability(migration_prop)
  migration_prop

  delta_u <- rbinom(current_n, size=1, prob=migration_prop[,2]/rowSums(migration_prop))
  # 0 => u1, 1 => u2
